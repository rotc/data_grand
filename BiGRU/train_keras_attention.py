...
from two_layers_bigru_with_attention import BiGRUModel
...
model = BiGRUModel(FLAGS.vocab_size, FLAGS.embedding_size, FLAGS.attention_size, \
...
lr_decay = keras.callbacks.ReduceLROnPlateau(monitor='val_acc', factor=0.5, patience=3, \
        mode='max')
model_save_path = './model_cache/two_layers_bigru_with_attention.hdf5'
...
df_sub = pd.DataFrame({'id':test_ids, 'class':test_preds})
sub_names = ['id', 'class']
df_sub[sub_names].to_csv('../Submit/two_layers_bigru_with_attention_f1{}.csv'.format(eval_f1_score), \
        index=False)
